...
async fn action_variant_and_its_command_and_config_resolve_from_graph() {
    ...
    let command_digest = graph.cas().put(&command).await.unwrap();

    // The config: a Tree with one file entry, itself resolvable from
    // the graph.
    let file_digest = graph.cas().put(&cas::Bytes::from_static(b"threshold: 10")).await.unwrap();
    let config =
        fabric::Tree::new([("config.yaml".to_string(), fabric::Node::Blob(file_digest))], []);
    let config_digest = graph.cas().put(&config).await.unwrap();

    let function = fabric::Function::action(command_digest, config_digest);
    let function_digest = graph.cas().put(&function).await.unwrap();
    ...
    let resolved_function: fabric::Function =
        graph.cas().get(&function_digest).await.unwrap().unwrap();
    ...
    let resolved_command: fabric::Command =
        graph.cas().get(&resolved_command_digest).await.unwrap().unwrap();
    assert_eq!(resolved_command, command);

    let resolved_config: fabric::Tree =
        graph.cas().get(&resolved_config_digest).await.unwrap().unwrap();
    assert_eq!(resolved_config, config);

    // The file the config tree references is itself resolvable.
    assert_eq!(
        graph.cas().get(&file_digest).await.unwrap(),
...
async fn server_variant_and_its_command_and_config_resolve_from_graph() {
    ...
    // The config: a Tree with one file entry, itself resolvable from
    // the graph.
    let file_digest = graph.cas().put(&cas::Bytes::from_static(b"port: 8080")).await.unwrap();
    let config =
        fabric::Tree::new([("config.yaml".to_string(), fabric::Node::Blob(file_digest))], []);
    let config_digest = graph.cas().put(&config).await.unwrap();
