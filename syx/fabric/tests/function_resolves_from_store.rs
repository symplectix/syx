...
async fn action_variant_and_its_command_and_config_resolve_from_graph() {
    ...
    let resolved_function: fabric::Function = store.get(&function_digest).await.unwrap().unwrap();
    ...
    let resolved_config: fabric::Tree = store.get(&resolved_config_digest).await.unwrap().unwrap();
