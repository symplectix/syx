...
async fn copy_from_file_produces_the_same_digest_as_put() {
    ...
    let from_reader = graph.copy_from(len, &mut file).await.unwrap();
    let from_bytes = graph.put(&cas::Bytes::from_static(b"hello")).await.unwrap();
...
async fn copy_from_bytes_produces_the_same_digest_as_put() {
    ...
    assert_eq!(d, cas_testing::digest_bytes(b"hello"));
    assert_eq!(graph.get(&d).await.unwrap(), Some(content));
}
